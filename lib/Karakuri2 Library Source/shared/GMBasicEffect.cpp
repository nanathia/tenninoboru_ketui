    ...
    : GMEffect("BasicEffect.vsh", "BasicEffect.fsh")
{
    ...
    mIsFogEnabled = false;
    mFogColor = GMColor::Black;
    mFogMode = GMFogModeExp;
    mFogNearZ = 0.0;
    mFogFarZ = 1.0;
    mFogDensity = 1.0;
...
void GMBasicEffect::enableFog(bool flag)
{
    mIsFogEnabled = flag;
...
GMColor GMBasicEffect::getFogColor() const
{
    return mFogColor;
}

double GMBasicEffect::getFogDensity() const
{
    return mFogDensity;
}

GMFogMode GMBasicEffect::getFogMode() const
{
    return mFogMode;
}

double GMBasicEffect::getFogRangeFarZ() const
{
    return mFogFarZ;
}

double GMBasicEffect::getFogRangeNearZ() const
{
    return mFogNearZ;
}

bool GMBasicEffect::isFogEnabled() const
{
    return mIsFogEnabled;
}

void GMBasicEffect::setFogColor(const GMColor& color)
{
    mFogColor = color;
    float the_color[4] = { color.r, color.g, color.b, color.a };
    glFogfv(GL_FOG_COLOR, the_color);
}

void GMBasicEffect::setFogDensity(double density)
{
    mFogDensity = density;
    glFogf(GL_FOG_DENSITY, (float)mFogDensity);
}

void GMBasicEffect::setFogMode(GMFogMode mode)
{
    mFogMode = mode;

    if (mFogMode == GMFogModeExp) {
        glFogi(GL_FOG_MODE , GL_EXP);
    } else if (mFogMode == GMFogModeExp2) {
...
void GMBasicEffect::setFogRange(double nearZ, double farZ)
{
    mFogNearZ = nearZ;
    mFogFarZ = farZ;

    glFogf(GL_FOG_START, (float)mFogNearZ);
    glFogf(GL_FOG_END, (float)mFogFarZ);
}
...
void GMBasicEffect::setLightAmbientColor(int index, const GMColor& color)
{
    __GMSetLightfv(GL_AMBIENT, index, color.toVector3D());
}
...
void GMBasicEffect::setLightDiffuseColor(int index, const GMColor& color)
{
    __GMSetLightfv(GL_DIFFUSE, index, color.toVector3D());
}

void GMBasicEffect::setLightSpecularColor(int index, const GMColor& color)
{
    __GMSetLightfv(GL_SPECULAR, index, color.toVector3D());
}
...
void GMBasicEffect::setLightTypeAsSpot(int index, const GMVector3D& vecPosition, const GMVector3D& vecDirection, double radAngle, double exponent)
{
    ...
    __GMSetLightf(GL_SPOT_CUTOFF, index, (GLfloat)GMMath::ToDegrees(radAngle / 2));
...
GMColor GMBasicEffect::getMaterialAmbient() const
{
    return mMaterial.ambient;
}

GMColor GMBasicEffect::getMaterialDiffuse() const
{
    return mMaterial.diffuse;
}

GMColor GMBasicEffect::getMaterialEmission() const
{
...
GMColor GMBasicEffect::getMaterialSpecular() const
{
