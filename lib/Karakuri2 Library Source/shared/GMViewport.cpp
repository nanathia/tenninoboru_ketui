...
GMViewport::GMViewport(const GMRect2D& bounds)
    : x((int)bounds.x), y((int)bounds.y), width((int)bounds.width), height((int)bounds.height), minDepth(0.0), maxDepth(0.0)
{
    // Do nothing
}
...
GMVector3D GMViewport::unproject(const GMVector3D& source, const GMMatrix& projection, const GMMatrix& view, const GMMatrix& world)
{
    ...
    if (fabs(maxDepth - minDepth) < GMMath::kDoubleEqThreshold) {
        result.z = 0.0;
    } else {
