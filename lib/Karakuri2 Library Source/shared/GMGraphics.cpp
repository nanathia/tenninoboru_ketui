...
GMGraphics* GMGraphics::CurrentGraphics = 0;
GMGraphics* GMGraphics::Graphics1 = 0;
GMGraphics* GMGraphics::Graphics2 = 0;


GMGraphics::GMGraphics()
{
    if (!GMGraphics::Graphics1) {
...
void GMGraphics::setRenderTarget(GMRenderTarget2D* target)
{
    ...
    if (!mRenderTarget) {
        ...
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, mCurrentMainFramebuffer);
#else
        std::cerr << "GMGraphics::setRenderTarget(GMRenderTarget2D*) - FBO is not supported!!" << std::endl;
#endif
    } else {
#if GL_ARB_framebuffer_object
        glBindFramebuffer(GL_FRAMEBUFFER, mRenderTarget->__getFramebuffer());
#elif GL_EXT_framebuffer_object
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, mRenderTarget->__getFramebuffer());
#else
        std::cerr << "GMGraphics::setRenderTarget(GMRenderTarget2D*) - FBO is not supported!!" << std::endl;
#endif
    }
