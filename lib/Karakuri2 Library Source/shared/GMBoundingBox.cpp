...
GMContainmentType GMBoundingBox::contains(const GMVector3D& point) const
{
    ...
    {
        return GMContainmentTypeDisjoint;
    } else if (fabs(point.x - min.x) < GMMath::kDoubleEqThreshold ||
               fabs(point.x - max.x) < GMMath::kDoubleEqThreshold ||
               fabs(point.y - min.y) < GMMath::kDoubleEqThreshold ||
               fabs(point.y - max.y) < GMMath::kDoubleEqThreshold ||
               fabs(point.z - min.z) < GMMath::kDoubleEqThreshold ||
               fabs(point.z - max.z) < GMMath::kDoubleEqThreshold)
    {
        return GMContainmentTypeIntersects;
    } else {
        return GMContainmentTypeContains;
...
{
    return equals(box, GMMath::kDoubleEqThreshold);
}
