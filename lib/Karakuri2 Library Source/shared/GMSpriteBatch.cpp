...
void GMSpriteBatch::begin(GMSpriteSortMode sortMode)
{
    mSortMode = sortMode;
...
void GMSpriteBatch::draw(GMTexture2D* tex, const GMRect2D& destRect, double layerDepth)
{
    draw(tex, destRect, GMRect2D::Empty, GMColor::White, layerDepth);
}
...
void GMSpriteBatch::draw(GMTexture2D* tex, const GMRect2D& destRect, const GMRect2D& srcRect, double layerDepth)
{
    draw(tex, destRect, srcRect, GMColor::White, 0.0, GMVector2D::Zero, GMSpriteFlipNone, layerDepth);
}
...
void GMSpriteBatch::draw(GMTexture2D* tex, const GMVector2D& pos, double layerDepth)
{
    draw(tex, pos, GMRect2D::Empty, GMColor::White, layerDepth);
}
...
void GMSpriteBatch::draw(GMTexture2D* tex, const GMVector2D& pos, const GMRect2D& srcRect, double layerDepth)
{
    draw(tex, pos, srcRect, GMColor::White, 0.0, GMVector2D::Zero, 1.0, GMSpriteFlipNone, layerDepth);
}
...
void GMSpriteBatch::fill(const GMRect2D& rect, const GMColor& color, double layerDepth)
{
    draw(0, rect, color, layerDepth);
}
...
static bool compareSpriteBackToFront(GMSprite* first, GMSprite* second)
{
    if (first->layerDepth > second->layerDepth) {
        return true;
    } else if (first->layerDepth < second->layerDepth) {
        return false;
...
void GMSpriteBatch::__flushBatchDraws()
{
    ...
    // スプライト描画の実行
    GMDraw::Begin();
    effect->enableTexture(false);
    GMTexture2D* currentTex = 0;
    for (std::list<GMSprite*>::iterator it = mSpriteList.begin(), it_end = mSpriteList.end(); it != it_end; it++) {
        ...
        GMVector2D texSize = GMVector2D::One;
        if ((*it)->tex) {
            texSize = (*it)->tex->getSize();
        }
        GMVector2D texScale = GMVector2D::One;
        if ((*it)->tex) {
            texScale = (*it)->tex->getScale();
        }
        ...
        if (tex != currentTex) {
            GMDraw::End();
            GMDraw::Begin();
            effect->enableTexture((tex != 0)? true: false);
