...
namespace startdemo{
    ...
    m_state(0){
        ...
        m_spk_win->addString("私は、漸この画面を見てくれている皆を歓迎するわ。");
        m_spk_win->addString("さて、今から始めるゲームは漸「天に昇る決意」。漸ある不幸を被った少女サクマが、漸立派に成長するまでの内容よ。");
        m_spk_win->addString("どんな物語か？漸そうね。それはやってからのお楽しみよ。冒頭だけ教えておくわ。", nextScene);
        m_spk_win->addString("");
        m_spk_win->addString("");
        m_spk_win->addString("");
        m_spk_win->addString("");
