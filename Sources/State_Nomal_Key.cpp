...
State_Nomal_Key::State_Nomal_Key(){
...
State_Key* State_Nomal_Key::update_key(GMInput* input, double deltaTime){
    State_Key* next = this;
    ...
    if(SKKeyHandle::m_flags&SKKeyHandle::key_down_once ||
       SKKeyHandle::m_flags&SKKeyHandle::key_up_once ||
       SKKeyHandle::m_flags&SKKeyHandle::key_right_once ||
       SKKeyHandle::m_flags&SKKeyHandle::key_left_once ||
       ...
       SKKeyHandle::m_flags&SKKeyHandle::key_return){
    }else if(input->isKeyDown(GMKeyMaskW)){
        next = new State_W_Key();
    }else if(input->isKeyDown(GMKeyMaskX)){
        next = new State_Skip_Key();
    }else if(gPlayScene->getUI()->isMenuOpen()){
        next = new State_Menu_Key();
    }
